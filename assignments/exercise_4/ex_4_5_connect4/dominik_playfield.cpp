...
dominik_playfield::dominik_playfield() {
    initialize();
}
...
int dominik_playfield::stoneat(int x, int y) const {
    return rep[x][y];
}

// display the dominik_playfield
void dominik_playfield::display() {
    cout << endl;
...
//void dominik_playfield::drop(int player) {
//    // todo Maybe change choice!!!
//
//    int choice;
...
void dominik_playfield::drop_at(int x, int player) {
    int choice = x;
    cout << "\nPlayer" << player << ", please select a number from 1 - 7: ";
    ...
    int number = 0;
    while (rep[(choice)][(height - 1) - number] != none) {
        number++;
        if (number > (height - 1)) {
            cout << "\nPlease select again: ";
            cin >> choice;
    ...
    rep[choice][(height - 1) - number] = player;
    last_move_x = choice;
    last_move_y = (height - 1) - number;
}
...
// initialize game
void dominik_playfield::initialize() {
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            rep[x][y] = none;
        }
...
bool dominik_playfield::check_diagonal_NW_SE(int x, int y, int player) {
    int score = 1;
    int count = 1;
...
// check diagonals used in check
bool dominik_playfield::check_diagonal_NE_SW(int x, int y, int player) {
    int score = 1;
    int count = 1;
...
bool dominik_playfield::check_vertical(int x, int y, int player) {
    // only down
    int score = 1;
    int count = 1;
...
bool dominik_playfield::check_horizontal(int x, int y, int player) {
    int score = 1;
    int count = 1;
    // left
...
bool dominik_playfield::check_won(int x, int y, int player) {
...
int dominik_playfield::get_last_move_x() {
    return last_move_x;
}

int dominik_playfield::get_last_move_y() {
    return last_move_y;
}
