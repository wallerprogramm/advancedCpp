...
inline void helper() {
  cout << endl
		...
		<< "  q           --  quit" << endl << endl
		<< "Fractions can be entered as: (cntr/denom)" << endl
        << "Complex numbers can be entered as: (RE,IM) where RE and IM doubles" << endl << endl;
}

int main(int argc, char const *argv[]) {
  string datatype;
  cout << "Welcome to the rpn calculator. Enter int, double, fraction or complex." << endl;
  cin >> datatype;
  if (datatype == "int") {
      ...
      RPN<complex<double> > rpn("/tmp/pvectorstack_complex.txt");
      helper();
      rpn.run_rpn();
  }
  else {
