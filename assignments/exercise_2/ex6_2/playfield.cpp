...
void playfield::drop(int player) {
    ...
    int number = 0;
    while (rep[(choice - 1)][(height - 1) - number] != ' ') {
        number++;
        if (number > (height - 1)) {
            cout << "\nPlease select again: ";
            cin >> choice;
    ...
    rep[choice - 1][(height - 1) - number] = player_symbol(player);
...
bool playfield::check_diagonal_NW_SE(int x, int y, int player) {
    ...
    while ((x - count >= 0) && (y + count >= 0)) {
        if (rep[x - count][y - count] == player_symbol(player)) { //Check direction NW
            score++;
            count++;
    ...
    while ((x + count < width) && (y + count < height)) {
        if (rep[x + count][y + count] == player_symbol(player)) { //Check direction SE
            score++;
            count++;
        } else break;  //If no combo is detected break from the loop
    }
...
bool playfield::check_diagonal_NE_SW(int x, int y, int player) {
    ...
    while ((x + count < width) && (y - count >= 0)) {
        if (rep[x + count][y - count] == player_symbol(player)) { //Check direction NE
            score++;
            count++;
        } else break;  //If no combo is detected break from the loop
    }
    ...
    while ((x - count >= 0) && (y + count < height) ) {
        if (rep[x - count][y + count] == player_symbol(player)) { //Check direction SW
            score++;
            count++;
        } else break;  //If no combo is detected break from the loop
    }
...
bool playfield::check_vertical(int x, int y, int player) {
    ...
    while ((y + count >= 0) && (y + count < height)) {
        if (rep[x][y + count] == player_symbol(player)) { //Check South
            score++;
            count++;
        } else break;  //If no combo is detected break from the loop
    }
...
bool playfield::check_horizontal(int x, int y, int player) {
    ...
    while ((x + count >= 0) && (x + count < width)) {
        if (rep[x + count][y] == player_symbol(player)) {  //Check East
            score++;
            count++;
        } else break;  //If no combo is detected break from the loop
    }
    ...
    while ((x - count < height) && (x - count >= 0)) {
        if (rep[x - count][y] == player_symbol(player)) { //Check West
            score++;
            count++;
        } else break;  //If no combo is detected break from the loop
    }
